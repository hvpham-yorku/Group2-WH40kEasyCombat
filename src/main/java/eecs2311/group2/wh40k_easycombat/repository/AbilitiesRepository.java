...
public class AbilitiesRepository {
	...
	public static int addNewAbilities(Abilities abilities) throws java.sql.SQLException {
		return Dao.update(
			"INSERT INTO Abilities (auto_id, id, faction_id, name, legend, description) VALUES (?, ?, ?, ?, ?, ?)",
			abilities.auto_id(),
			abilities.id(),
	...
	public static Abilities getAbilitiesByPk(int auto_id) throws java.sql.SQLException {
		return Dao.query(
			"SELECT * FROM Abilities WHERE auto_id = ?",
			rs -> new Abilities(
				rs.getInt("auto_id"),
				rs.getString("id"),
	...
	public static void updateAbilities(Abilities abilities) throws java.sql.SQLException {
		Dao.update(
			"UPDATE Abilities SET id = ?, faction_id = ?, name = ?, legend = ?, description = ? WHERE auto_id = ?",
			abilities.id(),
			abilities.faction_id(),
			abilities.name(),
			abilities.legend(),
	...
	public static void deleteAbilities(Abilities abilities) throws java.sql.SQLException {
		Dao.update(
			"DELETE FROM Abilities WHERE auto_id = ?",
			abilities.auto_id()
		);
	}
