...
public final class MissionCardRenderHelper {

    private MissionCardRenderHelper() {
    }

    public static void applyHeaderStyle(VBox headerBox, MissionType missionType) {
        headerBox.getStyleClass().removeAll("mission-primary-header", "mission-secondary-header");
        headerBox.getStyleClass().add(missionType == MissionType.PRIMARY
                ? "mission-primary-header"
                : "mission-secondary-header");
    }

    public static void renderBody(VBox missionBodyBox, MissionCard card) {
        missionBodyBox.getChildren().clear();
        if (card.type() == MissionType.PRIMARY) {
            renderPrimaryBody(missionBodyBox, card.bodyLines());
            return;
        }
        renderSecondaryBody(missionBodyBox, card.bodyLines());
    }

    private static void renderPrimaryBody(VBox missionBodyBox, List<String> lines) {
    ...
    private static void renderSecondaryBody(VBox missionBodyBox, List<String> lines) {
    ...
    private static Node buildSecondarySectionNode(SecondarySection section) {
    ...
    private static List<SecondarySection> parseSecondarySections(List<String> lines) {
    ...
    private static SecondarySection buildSecondarySection(String header, List<String> lines) {
        String whenLine = "";
    ...
    private static void flushSecondaryRow(List<SecondaryRow> rows, StringBuilder description, StringBuilder score) {
    ...
    private static Label buildBanner(String text, String styleClass) {
    ...
    private static Label buildCenteredLabel(String text, String styleClass) {
    ...
    private static Label buildBulletLabel(String text) {
        Label label = new Label("\u2022 " + text);
    ...
    private static Node buildScoreNode(String scoreText) {
        VBox box = new VBox(2);
    ...
    private static TextFlow buildTaggedParagraph(String line) {
    ...
    private static boolean isPrimaryBanner(String line) {
        String normalized = line == null ? "" : line.trim();
    ...
    private static boolean isSecondaryHeader(String line) {
        return line != null && line.toUpperCase(Locale.ROOT).contains("VICTORY POINTS");
    }

    private static String extractSecondaryHeader(String line) {
        return line.toUpperCase(Locale.ROOT)
                .replace("VICTORY POINTS", "")
                .trim();
    }

    private static boolean looksLikeVictoryLine(String line) {
        return line != null && line.matches("^[+]?\\d+VP.*");
    }

    private static boolean isScoreFragment(String line) {
        if (line == null || line.isBlank()) {
            return false;
        }
    ...
    private static boolean isOrLine(String line) {
